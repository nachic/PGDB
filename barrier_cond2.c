...
int main()
{
   ...
   pthread_create( &thread_id[2], NULL, thread_function3, NULL );
...
void *thread_function2(void *dummyPtr)
{
     printf("Thread number %ld\n", pthread_self());
     x++;
     printf("Function2 before barrier: x = %d\n",x);
     pthread_barrier_wait(&bar);
     printf("Function2 after barrier: x = %d\n",x);
...
void *thread_function3(void *dummyPtr)
{
     printf("Thread number %ld\n", pthread_self());
     x=2;
     printf("Function3 no barrier: x = %d\n",x);
}
